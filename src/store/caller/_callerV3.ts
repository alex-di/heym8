...
import { ICaller, IoEventType } from './types';
...
export class Caller extends EventEmitter implements ICaller {
    ...
    private mediaConstraints = {
      ...
      public setUsername(myUsername: string) {
        ...
        this.io.emit(IoEventType.Username, {
          name: myUsername,
          date: Date.now(),
          id: this.clientID,
        });
      ...
      private connect() {
        ...
        serverUrl = process.env.ROUTER_URI || "https://heym8-router-3ae6aec9f735.herokuapp.com";
        ...
        // this.connection.onerror = (evt) => {
          //   console.dir(evt);
          // }
        this.io.on(IoEventType.Id, (msg) => {
          this.emit(StoreEvent.CONN_OPEN, msg)
          console.log("ID", msg)
          this.clientID = msg.id;
          this.setUsername(this.username);
        })
      
        this.io.on(IoEventType.Userlist, (msg) => {
          this.handleUserlistMsg(msg);
        })
        
        this.io.on(IoEventType.VideoAnswer, (msg) => {

          this.handleVideoAnswerMsg(msg);
        })
        
        this.io.on(IoEventType.VideoOffer, (msg) => {
          this.handleVideoOfferMsg(msg);
        })
        
        this.io.on(IoEventType.NewICECandidate, (msg) => {
          this.handleNewICECandidateMsg(msg);
        })
        this.io.on(IoEventType.Hangup, (msg) => {
          this.handleHangUpMsg(msg);
        })
        this.io.on(IoEventType.Username, (msg) => {
          this.emit(StoreEvent.MESSAGE, JSON.stringify({ type: MessageType.USER_JOINED, time: new Date, name: msg.name}))
        })
        
        this.io.on(IoEventType.CallerMessage, (msg) => {
          this.emit(StoreEvent.MESSAGE, JSON.stringify({ type: MessageType.USER_MESSAGE, time: new Date, name: msg.name, text: msg.text }))
        })
        this.io.on(IoEventType.Note, (msg) => {
          this.emit(StoreEvent.NOTE, JSON.parse(msg.text))
        })
        
        this.io.on(IoEventType.Reconnect, (msg) => {
          this.emit(StoreEvent.RECONNECT, msg.text)

        })
        this.io.on(IoEventType.RejectUsername, (msg) => {
          console.log("REJECT USERNAME", msg)
        })
      ...
      public sendText(text: string) {
        ...
        this.io.emit(IoEventType.CallerMessage, msg);
      }
      ...
      public sendNote(note, octave) {
        ...
        this.io.emit(IoEventType.Note, msg);
      }
      ...
      public sendReconnect(user) {
        ...
        this.io.emit(IoEventType.Reconnect, msg);
      }
      ...
      private async handleNegotiationNeededEvent(user) {
        ...
        try {
          ...
          this.io.emit(IoEventType.VideoOffer, {
      ...
      private handleICECandidateEvent(user, event) {
        if (event.candidate) {
          ...
          this.io.emit(IoEventType.NewICECandidate, {
            target: user,
            name: this.username,
      ...
      public hangUpCall() {
        ...
        this.io.emit(IoEventType.Hangup, {
          name: this.myUsername,
      ...
      private async handleVideoOfferMsg(msg) {
        ...
        this.io.emit(IoEventType.VideoAnswer, {
          name: this.myUsername,
          target: user,
          sdp: this.peers[user].localDescription
        });
