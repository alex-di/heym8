...
function createKeyboard({ onNote }){
	...
	for(var k in keyboard) {
		...
		var val;
		switch(Number(k)) { //some characters don't display like they are supposed to, so need correct values
		...
		reverseLookupText[keyboard[k]] = val;
		reverseLookup[keyboard[k]] = k;
		...
		// Detect keypresses, play notes.
			...
			var fnPlayKeyboard = function(e) {
				...
				if(keyboard[e.keyCode]) {
					...
					onNote( note, __octave + octaveModifier)
